...
**Used at:** the Affiliates clause ([CUSTOMER]'s rewrite would have created a [PARENT_GROUP_EXAMPLE] Group backdoor; [VENDOR]'s reinstate-with-invitation forces [CUSTOMER] to disclose without naming [PARENT_GROUP_EXAMPLE]).
...
**Used at:** the DPA Updates clause ([VENDOR] accepted [CUSTOMER]'s consent gate, tightened the trigger to "materially adversely" and scoped it to "under this Agreement").
...
**Used at:** the Third Party Products clause (AS IS / AS AVAILABLE is the standard SaaS vendor position; [VENDOR] offers responsibility for non-negligent selection and [VENDOR]'s own integration code only).
...
**Used at:** the Affiliates clause ([PARENT_GROUP_EXAMPLE] angle stays internal, [CUSTOMER] has to surface their parent themselves).
...
**Used at:** the [VENDOR_EXAMPLE] IP clause ([USER_NAME] accepted [CUSTOMER]'s "for" → "on behalf of" change as a low-cost technical concession while holding firm on rejecting the Joint Workflows carve-out and reinstating the sub-clause [CUSTOMER] had struck).
...
**Used at:** the [VENDOR_EXAMPLE] IP clause ([USER_NAME]'s argument: Platform tooling is uniform across customers, customer USE is unique, therefore [CUSTOMER]'s "Joint Workflows" carve-out is mitigating a non-issue because customer use is not customer IP).
...
Established during review of the AS-IS disclaimer.

| Section type | Architectural role | Typical home in the MSA |
|---|---|---|
| **Operational commitment** | [VENDOR] positively commits to provide the Service | operations / delivery clauses |
| **Warranty** | [VENDOR] represents and warrants performance standard | warranty clause |
| **Risk allocation (disclaimers)** | What [VENDOR] is NOT promising | disclaimer clause |
| **Risk allocation (caps)** | Limits on [VENDOR]'s damages exposure | liability-cap clause |
| **Risk allocation (indemnity)** | Who covers whom for third-party claims | indemnity clause |

**Rule:** positive [VENDOR] commitments belong in the operations and warranty clauses. They do NOT belong in the disclaimer, liability-cap, or indemnity clauses.

**Why this matters:** when a customer pushes positive [VENDOR] commitments INTO the risk-allocation sections (e.g., adding a counter-warranty to the disclaimer clause to "balance" the AS-IS), the result is structurally incoherent. A court is then asked to choose between contradictory clauses. The clean answer is to relocate the positive commitment to the right architectural home (operations, warranties), or to reject it if it does not belong anywhere in the agreement.

**Used at:** the AS-IS disclaimer pushback ([CUSTOMER] added a safeguards warranty in the disclaimer clause; [VENDOR]'s structural answer is to either relocate to the warranty clause if substantively acceptable, or reject the substance and explain why a warranty is the wrong mechanism for AI output safeguards).
...
### Mechanism: AS-IS disclaimer for AI hallucinations

The AS-IS disclaimer does not exclude [VENDOR]'s responsibility for the Platform. It distinguishes between the SERVICE and individual OUTPUTS, which is the structural mechanism necessary for AI service delivery:
- **Service** = warranted in the warranty clause (Platform performance in accordance with Documentation)
- **Output** = allocated in the disclaimer clause (cannot be warranted because generative AI is probabilistic by design)
...
**Used at:** response to [CUSTOMER]'s high-risk concern on AI hallucination outputs.

### Mechanism: loss-of-data carve-out

"Loss of data" sits in the liability-cap exclusions as a categorical exclusion because data loss damages are by nature consequential. Their value is measured by downstream regulatory, reconstruction, and business-interruption effects, not by direct contractual valuation. Putting "loss of data" inside the direct-damages path would create unbounded valuation exposure for [VENDOR] while bypassing the structured data protection framework that lives in the DPA (security and incident-notification provisions), the MSA's customer-backup responsibility, and the indemnification for [VENDOR]'s gross negligence in data handling (which is uncapped).

**Used at:** response to [CUSTOMER]'s strike of "DATA" from the consequential damages list.
...
**Used at:** the Authorized Sub-Processor definition in the DPA ([CUSTOMER] changed "know or otherwise access" → "process" to align with GDPR Article 4(2); [VENDOR] accepted).
...
- [ ] **Strip the 14 bad-format Mage comments** from the Mage Comments working docx once [USER_NAME] confirms (those were internal-planning style; they should not be in a customer-facing file)
